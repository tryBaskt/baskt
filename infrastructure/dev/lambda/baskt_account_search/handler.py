"""Synchronize Baskt account DynamoDB stream records to OpenSearch."""
...
def _search_document(item: Dict[str, Any]) -> Dict[str, Any]:
    """Select only public Baskt account fields for the search index."""
    searchable_fields = (
        "cognito_user_id",
        "display_name",
        "description",
...
def lambda_handler(event: Dict[str, Any], context: Any) -> Dict[str, List[Dict[str, str]]]:
    ...
    for record in event.get("Records", []):
        ...
        if event_name in {"INSERT", "MODIFY"}:
            ...
            cognito_user_id = str(keys["cognito_user_id"])
            operations.append(
