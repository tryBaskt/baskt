#!/usr/bin/env python3
"""
Script to create or destroy the dev_model_portfolio_follower_dynamodb table.

Usage:
    python model_portfolio_follower_dynamodb.py --create   # Create the table
    python model_portfolio_follower_dynamodb.py --destroy  # Delete the table
"""
...
def create_dev_model_portfolio_follower_dynamodb():
    ...
    table_config = {
        ...
        'AttributeDefinitions': [
            ...
            {
                'AttributeName': configurations["sort_key"],
                'AttributeType': configurations["sort_key_attribute_type"]
            }
        ],
        'KeySchema': [
            {
            ...
            {
                'AttributeName': configurations["sort_key"],
                'KeyType': configurations["sort_key_key_type"]
            }
        ],
        'GlobalSecondaryIndexes': [
            {
                ...
                'KeySchema': [
                    ...
                    {
                        'AttributeName': gsi["gsi_sort_key"],
                        'KeyType': gsi["gsi_sort_key_type"]
                    }
                ],
                'Projection': {
                    'ProjectionType': 'ALL'
    ...
    try:
        ...
        for gsi in gsi_configs:
            print(
                f"GSI: {gsi['gsi_name']} "
                f"(PK: {gsi['gsi_partition_key']}, SK: {gsi['gsi_sort_key']})"
            )
...
def delete_dev_model_portfolio_follower_dynamodb():
    """
    Delete the dev_model_portfolio_follower_dynamodb table.
    """
    TABLE_NAME = configurations["table_name"]
    REGION = configurations["region"]
...
def main():
    ...
    if args.create:
        create_dev_model_portfolio_follower_dynamodb()
    elif args.destroy:
        delete_dev_model_portfolio_follower_dynamodb()
