...
Coverage goals:
- add_access_for_user(): resolve an existing shared user's email through
  Cognito and write the access record, then assert an unknown email raises
  ModelPortfolioAccessUserNotFoundError.
- add_access_for_user_by_cognito_user_id(): resolve an existing Cognito user ID
  and write the access record, then assert an unknown Cognito user ID raises
  ModelPortfolioAccessUserNotFoundError.
- remove_access_for_user(): delete an existing access record, assert removing a
  follower raises ModelPortfolioAccessUserIsFollowerError, and assert removing
  a missing access raises ModelPortfolioAccessNotFoundError.
...
- write_access_item_without_lock_by_cognito_user_id(): write an access record
...
- get_accesses_shared_with_user(): return no records for a user with no grants,
...
- get_accesses_granted_by_owner(): return no records for an owner with no
  grants and multiple records for an owner after granting access to multiple
  portfolios.
...
def test_model_portfolio_access_repository_add_access_for_user_success_and_missing_email(
...
def test_model_portfolio_access_repository_remove_access_success_follower_and_missing(
    ...
    """Remove access, block follower removal, and raise for missing access."""
...
def test_model_portfolio_access_repository_write_without_lock_under_existing_lock(
    ...
    try:
        ...
        model_portfolio_access_repository.write_access_item_without_lock_by_cognito_user_id(
            portfolio_id=portfolio_id,
            portfolio_owner_cognito_user_id=test_user_1.cognito_user_id,
            shared_with_cognito_user_id=test_user_2.cognito_user_id,
        )
...
@pytest.mark.integration
def test_model_portfolio_access_repository_get_accesses_shared_with_user_counts_change(
    model_portfolio_access_repository: ModelPortfolioAccessRepository,
    test_user_1,
    test_user_2,
) -> None:
...
@pytest.mark.integration
def test_model_portfolio_access_repository_get_accesses_granted_by_owner_multiple_and_none(
    model_portfolio_access_repository: ModelPortfolioAccessRepository,
    test_user_1,
    test_user_2,
) -> None:
    """Return multiple owner-granted records and none for an owner with none."""
    portfolio_ids = [
        f"tests-v2-owner-access-one-{uuid4()}",
        f"tests-v2-owner-access-two-{uuid4()}",
    ]
    missing_owner_id = f"tests-v2-missing-owner-{uuid4()}"
