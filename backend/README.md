...
3. Making a Baskt- The user can look up stocks, add them to a Baskt, and have the backtest run whenever updates to the Baskt are made. In the Baskt, users can set the weight of the stock, long or short, and leverage (but this is set to 1 and not changeable for now). You can get the list of stocks from GET "/backtest/tradeable-fractionable-us-baskt-assets". You can get backtest results from GET "/backtest". The user can set the start date and end date for the backtest. The backtest should only run when the total weight of the baskt is at 100%. When its not 100%, the backtest box should be blank. You can save the Baskt through POST "/model-portfolios". This functionality can probably be its own page called MakeABaskt.jsx.
...
7. Transfering money between bank <-> account- The user can connect a ACH relationship and bank. They can connect at most one ACH and bank. For ACH, we can see the status, account number, account owner name, nickname, and bank_routing_number. For bank, the user should see status, name, bank_code, and account_number. Then, the user can deposit and withdraw money. The user can also update/change ACH or bank information as they need. Last, the user can see past transfers. The necessary methods should be available in account_lifecycle_route.py. This functionality can also be its own page called Transfer.jsx.
