  ...
  environment {
    variables = merge(
      var.trade_worker_environment,
      {
        ENV                                  = var.environment
        ALPACA_ENV                           = "sandbox"
        TEST_COGNITO_REGION                  = var.cognito_region
        TEST_COGNITO_USER_POOL_ID            = var.cognito_user_pool_id
        TEST_COGNITO_APP_CLIENT_ID           = var.cognito_app_client_id
        TEST_TRADE_EXECUTION_QUEUE_URL       = var.queue_url
